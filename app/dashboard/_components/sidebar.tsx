...
export const menu = [
  ...
  },
  {
    ...
    submenu: [
      ...
      { title: "Permissions", url: "/dashboard/user/permissions", icon: Users },
    ],
  },
];
          ...
          >
            {filteredMenu.map((item, index) => {
              ...
              if (!isSubmenu) {
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2 text-[13px] font-medium py-2 px-3 rounded-md no-underline transition-all ${pathname === item.url
              ...
              return (
                ...
                >
                  <AccordionTrigger
                    className={`flex items-center justify-between text-[13px] font-medium py-2 px-3 rounded-md no-underline hover:no-underline transition-all [&>svg]:hidden ${activeIndex === `item-${index}`
                      ? "bg-[#f2f2f2]"
                      : "hover:bg-[#f5f5f5]"
                      }`}
                  >
                    ...
                    <div className="flex items-center">
                      <ChevronDown
                        className={`w-4 h-4 transition-transform duration-200 ${activeIndex === `item-${index}` ? "rotate-180" : ""
                          }`}
                      />
                    </div>
                  </AccordionTrigger>
                        ...
                        if (subItem.submenu) {
                          ...
                          return (
                            <Accordion type="single" collapsible key={i} className="w-full">
                              <AccordionItem value={`sub-${i}`} className="border-0">
                                <AccordionTrigger
                                  className={`group flex items-center justify-between text-[12px] py-2 px-3 ml-4 rounded-md no-underline hover:no-underline transition-all [&>svg]:hidden text-[#737373] hover:bg-[#f5f5f5] hover:text-black`}
                                >
                                  <div className="flex items-center gap-2">
                                    {subItem.icon && <subItem.icon className="w-3.5 h-3.5 shrink-0" />}
                                    <span className="no-underline">{subItem.title}</span>
                                  </div>
                                  <div className="flex items-center">
                                ...
                                <AccordionContent className="mt-1">
                                  <div className="flex flex-col gap-1">
                                    {subItem.submenu.map((nestedItem, j) => (
                                      <Link
                                        key={j}
                                        href={nestedItem.url}
                                        onClick={mobileSidebar}
                                        className={`flex items-center text-[12px] py-2 px-3 ml-8 rounded-md no-underline transition-all ${
                          ...
                          return (
                            <Link
                              ...
                              href={subItem.url}
                              onClick={mobileSidebar}
                              className={`flex items-center gap-2 text-[12px] py-2 px-3 ml-4 rounded-md no-underline transition-all ${
